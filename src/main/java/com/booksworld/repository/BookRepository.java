...
public interface BookRepository extends JpaRepository<Book, Long> {
	...
	@Query(value = "select * from BOOK  where BOOK.BOOK_NAME = :name", nativeQuery = true)
	List<Book> findBybook_name(@Param("name") String name);

	@Query(value = "select * from BOOK  where BOOK.USER_ID = :userid and BOOK.BOOK_STATUS = :status", nativeQuery = true)
